...
\begin{frame}
  ...
  slope/intercept values. Intersection points stored in a C++ STL map
  (red-black tree, sorted by step size), $O(\log N)$ insertion, $O(1)$
  lookup of next intersection. Overall, worst case $O(N)$ space,
  \begin{description}
  \item[grid: standard grid search.] Linear $O(N\log N)$ time per step, but
    potentially slow for a sub-optimal grid search.
  \item[exact.linear: only first $N$ line search iterations.] Linear
    $O(N\log N)$ time per step, relatively small step sizes chosen,
    relatively large number of steps overall in gradient descent.
  \item[exact.quadratic: all line search iterations.] Quadratic
    $O(N^2)$ intersections, $O(N^2\log N)$ time per step, large step
    sizes, small number of steps.
  \item[exact.first.min: keep doing line search iterations until first AUM
    increase.] Same as exactQ time in worst case, but may be faster on
    average.
  \end{description}
\end{frame}
