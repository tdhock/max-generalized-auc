...
\begin{frame}
  ...
  \vskip -0.2cm
  \begin{itemize}
  \item Let $\mathbf{\hat y}$ be the $N$-vector of predicted scores,
  \item with up to $N$ points on the ROC curve,
  \item with sorted thresholds $-\infty=\tau(\mathbf{\hat y})_0<\cdots<\tau(\mathbf{\hat y})_N=\infty\in\mathbb R$,
  \item and corresponding min error values $m(\mathbf{\hat y})_1,\dots,m(\mathbf{\hat y})_N$.
  \item Then the sum of the min (SM), over all ROC points, is:
  \end{itemize}
\begin{equation*}
  \label{eq:SM-computation}
    \text{SM}(\mathbf {\hat y}) =
    \sum_{i=2}^{N-1}
    I[ \tau(\mathbf {\hat y})_{i} \neq \tau(\mathbf {\hat y})_{i-1} ]
    m(\mathbf {\hat y})_i =
    \sum_{i:\tau(\mathbf {\hat y})_{i} \neq \tau(\mathbf {\hat y})_{i-1} }
    m(\mathbf {\hat y})_i.
\end{equation*}

($\neq$ required: tied scores in $\mathbf{\hat y}$ delete points from the ROC curve)
...
\begin{eqnarray*}
    ...
    \sum_{i=2}^{N-1}
    [ \tau(\mathbf {\hat y})_{i} - \tau(\mathbf {\hat y})_{i-1} ]
                                     m(\mathbf {\hat y})_i.\\
\end{eqnarray*}
...
\begin{frame}
  ...
  \item Proposed AUM loss similar to recent losses that sum over all
    ...
    different/novel relaxation similar to ROC curve computation.
  \item Proposed AUM loss implemented in 15 lines of pytorch code, can be used as a drop-in replacement for logistic/binary cross-entropy loss, \url{https://tdhock.github.io/blog/2022/aum-learning/}
  \item Best way to use with stochastic gradient algorithms?
