...
\section{ROC curve optimization using gradient descent}

\begin{frame}
  \frametitle{Comparing logistic loss and rank/AUM loss}
  \begin{itemize}
  \small
  ...
  \item Bamber (1975) proved ROC-AUC relation to Mann-Whitney
    ...
    samples).
  \item Recently:  $\text{SVM}^{\text{struct}}$ (Joachims 2005) and X-risk (Yang 2022)
    %https://arxiv.org/abs/2206.00439
    sum loss over pairs of positive and negative samples, $\ell[f(x^+)-f(x^-)]$.
  \item Proposed: AUM loss (sum over points on ROC curve).
...
\begin{frame}
  \frametitle{L1 relaxation}
  \includegraphics[width=\textwidth]{figure-more-than-one-binary-heat}
Let $\hat y$ be the $N$-vector of predicted scores, making $Q$ points on the ROC curve, with sorted thresholds $\tau$, and min(FPR,FNR) values $m$. Then the sum of the min over all ROC points is:
\begin{equation*}
...
The AUM can be interpreted as an L1 relaxation of the following non-convex \textbf{S}um of \textbf{M}in(FP,FN) function, 
