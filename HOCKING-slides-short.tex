...
\begin{frame}
  ...
  \item In binary classification each observation $i$ with a positive
    label has an error function which results in a false negative if
    $\hat y_i<0$.
  \item And each observation with a negative
...
\begin{frame}
  ...
  \item Breakpoint threshold,
    ...
    some $q$.
  \item Total label error (FP/FN/min) before/after each breakpoint can
    be computed via sort and modified cumsum:
  \end{itemize}
\begin{eqnarray*}
...
\begin{frame}
  ...
  \item Empirical results provide evidence that learning using AUM
    ...
    monotonic/regular curves with large AUC but no loops).
